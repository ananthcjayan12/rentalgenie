...
def calculate_rental_amounts(doc):
    ...
    # Calculate total rental amount from items
    for item in doc.items:
        if item.qty and item.rate:
            item_total = flt(item.qty) * flt(item.rate) * flt(doc.rental_duration_days or 1)
            item.amount = item_total
            total_rental += item_total
    
    # Calculate balance due on delivery
    advance = flt(doc.advance_amount or 0)
    doc.balance_due_on_delivery = flt(total_rental) - advance
...
def validate_exchange_booking(doc):
    ...
    # Transfer advance from original booking
    if not doc.advance_amount and original_booking.advance_amount:
        doc.advance_amount = original_booking.advance_amount
...
def on_submit_sales_invoice(doc, method):
    ...
    if doc.is_rental_booking:
        ...
        # Create owner commission entries for third-party items
        ...
        for item in doc.items:
            ...
            if item_doc.is_rental_item and item_doc.is_third_party_item and item_doc.owner_commission_percent:
                ...
                if commission_amount > 0:
                    # Create commission liability account if needed
                    commission_account = f"Owner Commission Payable - {company_abbr}"
                    if not frappe.db.exists("Account", commission_account):
                        account = frappe.get_doc({
                            "doctype": "Account",
                    ...
                    # Update commission liability in GL
                    # This will be handled by standard ERPNext GL entries
                    frappe.db.sql("""
                        INSERT INTO `tabGL Entry` (
                            posting_date, account, debit, credit, voucher_type, voucher_no,
                            company, remarks
                        ) VALUES (
                            %s, %s, 0, %s, 'Sales Invoice', %s, %s, %s
                        )
